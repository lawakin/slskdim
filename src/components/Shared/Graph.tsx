...
export type GraphSeries = {
  ...
  key: string;
  name: string;
  unit?: 'bytes' | 'count' | 'rate' | 'ratio' | 'seconds' | 'speed';
};

type GraphProps = {
  readonly data?: Array<Record<string, number>>;
  readonly defaultSeries?: Set<string>;
  readonly height?: number;
  readonly series?: GraphSeries[];
};

const UNIT_FORMATTERS: Record<
  NonNullable<GraphSeries['unit']>,
  (value: number) => string
> = {
...
const mixedFormatter = (v: number): string => {
...
const Graph = ({
  ...
  const yFormatter = useMemo(() => {
    ...
    return unique.length === 1
      ? (UNIT_FORMATTERS[unique[0] as NonNullable<GraphSeries['unit']>] ??
          mixedFormatter)
      : mixedFormatter;
  }, [series, visible]);
